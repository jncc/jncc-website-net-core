    ...
    function udContentflowComponent(workspaceHelper, angularHelper, deployQueueService, deployService, deployConfiguration, deploySignalrService, deployHelper, notificationsService) {
        ...
        function link(scope, element, attr, ctrl) {
            ...
            function updateLog(event, sessionUpdatedArgs) {
                ...
                if (deployService.isOurSession(sessionUpdatedArgs.sessionId) && isContentTransfer(sessionUpdatedArgs.workItem)) {
            ...
            function isContentTransfer(workItem) {
                if (!workItem) {
                    // Work item should be defined, but in an unlikely case of someone using a custom Signalr Hub that doesn't implement IDeployHub2, 
                    // this value will not be available.
                    return true;
                }

                return workItem === "SourceDeployWorkItem";
            }

            // signalR events for deploy progress
            scope.$on('deploy:sessionUpdated', function (event, args) {

                // make sure the event is for us and that it's for a content transfer
                if (args.sessionId === deployService.sessionId && isContentTransfer(args.workItem)) {
                    angularHelper.safeApply(scope, function () {
            ...
            scope.$on('deploy:heartbeat', function (event, args) {
                if (!deployService.isOurSession(args.sessionId) || !isContentTransfer(args.workItem)) return;
        ...
        function link(scope, element, attr, ctrl) {
            ...
            scope.startDeploy = function () {
                ...
                deployService.deploy(scope.enableWorkItemLogging, scope.deployInfo).then(function(data) {
                    ...
                    scope.$parent.deploy.timestamp = moment().format(timestampFormat);
...
(function () {
  ...
  angular
    .module('umbraco.deploy.components')
    .directive('udImportArtifactsError', udImportSchemaMismatchErrorComponent);

  function udImportSchemaMismatchErrorComponent(deployService) {
    function link(scope, element, attr, ctrl) {
      scope.prettyEntityType = function (udi) {
        return deployService.prettyEntityType(udi);
      }
    ...
    var directive = {
      restrict: 'E',
      replace: true,
      templateUrl: '/App_Plugins/Deploy/views/components/errors/udimportartifactserror/udimportartifactserror.html',
      scope: {
        'exception': "=",
        'feedbackMessageLevel': "="
      },
...
angular.module('umbraco.deploy.components').directive('udImportArtifactsUnhandledError', function () {
  return {
    restrict: 'E',
    replace: true,
    templateUrl: '/App_Plugins/Deploy/views/components/errors/udimportartifactsunhandlederror/udimportartifactsunhandlederror.html',
    scope: {
      'exception': "=",
      'feedbackMessageLevel': "="
    }
...
(function() {
    ...
    angular
        .module('umbraco.deploy.components')
        .directive('udRemoteApiTimeoutError', udRemoteApiTimeoutErrorComponent);

    function udRemoteApiTimeoutErrorComponent() {
        function link(scope, element, attr, ctrl) {
        }
        ...
        var directive = {
            restrict: 'E',
            replace: true,
            templateUrl: '/App_Plugins/Deploy/views/components/errors/udremoteapitimeouterror/udremoteapitimeouterror.html',
            scope: {
                'exception': "=",
                'feedbackMessageLevel': "=",
            },
...
(function () {
  ...
  angular
    .module('umbraco.deploy.components')
    .directive('udExportComplete', udExportCompleteComponent);

  function udExportCompleteComponent() {

    var directive = {
      restrict: 'E',
      replace: true,
      templateUrl: '/App_Plugins/Deploy/views/components/export/udexportcomplete/udexportcomplete.html',
      scope: {
        'onDownload': "&",
        'onDelete': "&",
        'timestamp': "=",
...
(function() {
    ...
    angular
        .module('umbraco.deploy.components')
        .directive('udExportProgress', udExportProgressComponent);

    function udExportProgressComponent() {

        var directive = {
            restrict: 'E',
            replace: true,
            templateUrl: '/App_Plugins/Deploy/views/components/export/udexportprogress/udexportprogress.html',
            scope: {
                'progress': "=",
                'currentActivity': "=",
                'timestamp': "=",
...
(function () {
  ...
  angular
    .module('umbraco.deploy.components')
    .directive('udImportComplete', udImportCompleteComponent);

  function udImportCompleteComponent() {

    var directive = {
      restrict: 'E',
      replace: true,
      templateUrl: '/App_Plugins/Deploy/views/components/import/udimportcomplete/udimportcomplete.html',
      scope: {
        'timestamp': "=",
        'serverTimestamp': "="
      }
...
(function() {
    ...
    angular
        .module('umbraco.deploy.components')
        .directive('udImportProgress', udImportProgressComponent);

    function udImportProgressComponent() {

        var directive = {
            restrict: 'E',
            replace: true,
            templateUrl: '/App_Plugins/Deploy/views/components/import/udimportprogress/udimportprogress.html',
            scope: {
                'progress': "=",
                'currentActivity': "=",
                'timestamp': "=",
