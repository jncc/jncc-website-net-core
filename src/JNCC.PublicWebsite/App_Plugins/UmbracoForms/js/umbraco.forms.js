...
angular.module("umbraco")
  .controller("UmbracoForms.Editors.Form.CopyController",
    function ($scope, formResource, navigationService, localizationService) {
...
(function () {
  ...
  function Controller($scope, formResource, navigationService, notificationsService, treeService, localizationService) {
...
angular.module("umbraco").controller("UmbracoForms.Editors.Form.EntriesController",
  function ($scope, $routeParams, recordResource, formResource, editorService, userService, securityResource, notificationsService, navigationService, overlayService, localizationService) {
    ...
    return {
      ...
      link: function (scope, element, attrs, ctrl) {
        ...
        scope.initForm = function (form, fieldtypes) {
          ...
          populateConditionStatusLabels(form);
        };
        ...
        var applyStatusLabel = function (condition, format) {
          if (condition) {
            condition.statusLabel = format
              .replace("%0%", condition.actionType)
              .replace("%1%", condition.logicType.toLowerCase());
          }
        };

        var populateConditionStatusLabels = function (form) {
          localizationService.localizeMany([
            "formConditions_pageButtonConditionStatus",
            "formConditions_fieldsetConditionStatus",
            "formConditions_fieldConditionStatus"]).then(function (labels) {
            angular.forEach(form.pages, function (page) {
              applyStatusLabel(page.buttonCondition, labels[0]);
              angular.forEach(page.fieldSets, function (fieldset) {
                applyStatusLabel(fieldset.condition, labels[1]);
                angular.forEach(fieldset.containers, function (container) {
                  angular.forEach(container.fields, function (field) {
                    applyStatusLabel(field.condition, labels[2]);
                  });
                });
              });
            });
...
(function () {
  ...
  function providerLocalizationHelper() {
    ...
    var getLocalizationKeysForFormWorkflows = function (formWorkflows) {
      var keys = [];
      var allWorkflows = formWorkflows.onSubmit;
      allWorkflows.push.apply(allWorkflows, formWorkflows.onApprove);

      var stem = "formProviderWorkflows";
      for (var i = 0; i < allWorkflows.length; i++) {
        var keyStem = stem + "_" + convertNameForKey(allWorkflows[i].workflowTypeName);
    ...
    var applyLocalizationLabelsToFormWorkflows = function (formWorkflows, labels) {
      ...
      for (var i = 0; i < allWorkflows.length; i++) {
        ...
        if (labelIsTranslated(labels[labelIndex])) {
          allWorkflows[i].workflowTypeName = labels[labelIndex];
        }
        labelIndex++;
        if (labelIsTranslated(labels[labelIndex])) {
          allWorkflows[i].workflowTypeDescription = labels[labelIndex];
        }
        labelIndex++;
      }
    };
