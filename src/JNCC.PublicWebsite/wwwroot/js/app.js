...
function initFoundation() {
    ...
    var mobileNav = $('#menu');
    var searchNavOpener = null;
    var mobileNavOpener = null;

    if (searchNav.length === 1) {
        searchNavOpener = $('[data-responsive-toggle="' + searchNav.attr('id') + '"]');
    }
    ...
    $(window).on('toggled.zf.responsiveToggle', (e) => {
        if (e.target.dataset.responsiveToggle == 'search-form') {
            if (searchNav.is(":visible")) {
                searchNav.on('focusout', function (e) {
                    if (e.relatedTarget) {
                        if ($(e.relatedTarget).parents('#search-form').length === 0 && searchNav.is(":visible")) {
                            searchNavOpener.foundation('toggleMenu');
                        }
                    }
                });
                $(window).on('keyup', function (e) {
                    if (e.which == 27 && searchNav.is(":visible")) {
                        searchNavOpener.foundation('toggleMenu');
                    }
                });
            }
            else {
                searchNav.off('focusout');
                $(window).off("keyup");
            }
        }
        if (e.target.dataset.responsiveToggle == 'menu') {
...
function initScienceDetailsSlickCarousel() {
    $('.science-details-image-gallery.slideshow').on('init', function (event, slick) {
    ...
    $('.science-details-image-gallery.slideshow').slick({
        dots: true,
        infinite: true,
        speed: 500,
        fade: true,
...
(function ($) {
    ...
    var $modalBody = $('#image-modal-body', $modal);
    ...
    $('.thumbnail-modal').each(function (index, item) {
        ...
        if ($img.length > 0) {
            ...
            imgCache[index] = $('<img src="' + $img.data("imageUrl") + '" alt="' + $img.attr("alt") + '" />');
        }
    ...
    $('.thumbnail-modal').on('click', function (e) {
        ...
        var $image = imgCache[index];

        if ($image.length === 0) {
            return;
        }

        var $body = $image.clone();
        $modalBody.html($body);
